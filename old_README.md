...
(Still updating, feel free to open an issue or submit a pull request. You can also reach out to me directly at k5shao@ucsd.edu.)
...
Two ways to run inference (defaults match previous hard-coded values):

1) Single pair: content file and style file

```bash
python ddsp_inference.py /path/to/src.wav /path/to/style.wav --ckpt_type mix --post_opt post_opt_0.2 --topk 4 --device cuda --prioritize_f0 true --tgt_loudness_db -16
```

2) Dataset mode: content folder and style folder (each containing speakers)

```bash
python ddsp_inference.py /path/to/src_speaker_folder /path/to/tgt_speaker_folder --ckpt_type mix --post_opt post_opt_0.2 --required_subset_file /path/to/split.csv --duration_limit 30
```

- If you pass `--with_dur_limit`, the script will iterate over {5,10,30,60,90} seconds for bulk conversion.
